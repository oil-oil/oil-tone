---
name: oil-tone
description: "为 oil 本人的中文或英文成稿提供文风规范，用于博客、演讲稿、PPT 文案、网站与产品介绍、个人简介、公众号和社交帖子。用户要求使用 oil-tone、用我的语气写，或文字需要代表 oil 本人时使用。不用于普通技术问答、代码修改、逐字转录、忠实翻译或其他人的署名文稿，除非用户明确指定。"
---
...
- 百分比、倍数等由原始数字换算得到的衍生数据，直接采用来源给出的数值，并写明口径，例如「按报告的测算」。自己的换算与来源不一致时，先核对分母、单位、时间范围和舍入方式；无法解释差异时保留来源归属并说明差异，不把未经核对的结果写成事实。
- 公开成稿里点名第三方产品的负面结果，即使来源可靠，也先向用户确认是否点名；默认只作中性罗列，不单独展开批评。
- 面向陌生读者介绍产品、项目、功能、Skill、命令或其他专有名词时，在第一次出现处用一句自然的话说明它是什么、与当前内容有什么关系。原文已经提供官网、仓库、安装页或原始资料链接时，把链接放到第一次介绍处；没有可靠来源时不猜测链接。
...
- 「A 把 X 和 Y 连接（整合、合并）起来」这类句子，动作的发出者 A 不能出现在对象 X、Y 里面，否则变成自己对自己做动作。省略宾语时，读者要能把宾语自然地补回来。
- 句子长短可以变化。不要连续堆叠碎句，也不要把多个判断压进结构复杂的长句。
- 朗读时应该像人在自然说明一件事情。不要依靠大量语气词模拟口语，也不要使用不符合日常语法的压缩表达。
- 需要口语节奏的成稿保留完整的虚词。「在、的、了、的时候」这类词承担语法和节奏作用，压缩掉以后朗读会发紧，例如「使用时」写成「使用的时候」、「仅」换成「只」。补全虚词让句子完整，和删除多余语气词是两件事，不要混在一起处理。

## 内容逻辑和结构

- 按照实际关系组织内容。材料包含原因、实际影响和后续做法时，把这些关系说明清楚，不删除中间步骤。
- 给主要判断提供必要的解释，但不要换一种说法重复同一结论。比较不同工具或方案时，直接说明具体能力、限制和适用情况。
- 同一数字、事实或结论不在相邻段落重复出现。先说明事实的段落和写反应、判断的段落各自分工，感想段直接写反应，不复述事实段已经出现的数字；收尾总结除外。
...
- 文内小标题当作内容标签，直接说明这一部分讲什么，不制造额外的文案感。面向读者的整篇文章标题可以表达材料已经支持的真实动作、变化、结果或作者判断，让读者知道点开后会看到什么。
- 整篇文章标题中的每个「我」、动作、时间、数量、结果和判断都必须有材料的直接证据。只有材料明确说明 oil 开发、制作、设计、新增或开源某个对象时，才能使用「我做了」「我开发了」「我开源了」等所有权表达。第三方产品只写已经证实的使用、测试、发现和判断。
- 无论是整篇标题还是小标题，都不要为了显得有观点、有节奏或有态度，在冒号、逗号、破折号后面自行添加行动建议、转折判断、对仗句或口号。
...
- **小红书、公众号短帖、朋友圈：** 可以使用较自然的口语节奏，面向读者时根据语境使用「我们」或「大家」，不虚构起因和场景。微信公众号文章的正文首段固定为「哈喽大家好，我是 oil 欧呦。」，问候语后另起一段进入内容；其他内容不套用这句固定开场。
...
`FAIL` 表示已经确认的 oil-tone 问题，修改后重新运行。`WARN` 只表示可能存在固定的 AI 表达，需要结合上下文判断；表达确有作用时可以保留。程序通过后，朗读全文并按照前述规则完成一次人工检查，重点是程序查不出的问题：朗读发紧的虚词缺失；相邻段落重复的数字或结论；长句的主语指代是否清楚、动作对象是否循环；数字、百分比与来源逐个核对。程序只能识别已知表达，不能代替人工判断。
